...
def goal_prompt_document_paths(prompt_path: Path, prompt_text: str) -> list[Path]:
    """Return existing markdown docs that should be surfaced with a prompt.

    /goal_prompt runbooks commonly list absolute "routine read-first" and
    operator-visible reference docs. Messaging users expect those files to
    appear in chat before the kickoff turn starts, so parse those paths in
    document order and fall back to the conventional project docs when a
    prompt uses relative prose only.
    """
    prompt_path = prompt_path.expanduser().resolve()
    project_root = prompt_path.parent.parent.parent if prompt_path.match("*/docs/runbooks/*") else prompt_path.parent
    ...
    def add(path: Path) -> None:
        try:
            ...
            return
        if resolved.exists() and resolved.is_file() and resolved.suffix.lower() == ".md" and resolved not in ordered:
            ordered.append(resolved)

    add(prompt_path)

    # Capture absolute markdown paths, including paths with spaces, up to .md.
    for line in (prompt_text or "").splitlines():
        for match in re.finditer(r"(?P<path>(?:~|/)[^`<>]*?\.md)\b", line, re.IGNORECASE):
            add(Path(match.group("path").strip()))

    fallback_docs = (
        project_root / "AGENTS.md",
        project_root / "README.md",
        project_root / "GOAL.md",
        project_root / "docs" / "status" / "NEXT_ACTIONS.md",
        project_root / "docs" / "security" / "SAFETY.md",
        project_root / "docs" / "runbooks" / "GOAL_PROMPT.md",
    )
