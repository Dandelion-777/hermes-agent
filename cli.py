...
class HermesCLI:
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from hermes_cli.goals import GoalManager, bind_session_db
            from hermes_cli.config import load_config
        except Exception as exc:
        ...
        try:
            bind_session_db(getattr(self, "_session_db", None))
        except Exception:
            pass
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        if state.goal_mode == "goal_prompt_oneshot":
            ...
            if state.compaction_refresh_interval > 0:
                _cprint(
                    f"  {_DIM}↻ Will refresh after {state.compaction_refresh_interval} "
                    f"compactions with a fresh session reload.{_RST}"
                )
        else:
