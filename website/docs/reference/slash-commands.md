...
| `/goal_prompt_oneshot [path]` | High-autonomy project-loop variant of `/goal_prompt`: uses a larger budget, requires deterministic final blocks with a `Judge reasoning:` line immediately before `CONTINUE` / `STOP_FOR_OPERATOR` / `COMPLETE`, shows judge reasoning + verdict and the exact next action between loops, and can restart through `/new` after repeated compactions. |
