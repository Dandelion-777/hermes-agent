...
def bind_session_db(db: Any) -> None:
    """Bind goal persistence to an existing SessionDB for this Hermes home.

    The CLI owns a SessionDB instance during tests and some embedded modes.
    Reusing it keeps ``/goal`` metadata in the same store as the active
    transcript instead of opening an unrelated default database.
    """
    if db is None:
        return
    try:
        ...
        _DB_CACHE[str(get_hermes_home())] = db
    except Exception:
        return
