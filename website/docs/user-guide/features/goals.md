...
4. **Loop fires if needed** — if `continue`, you'll see `↻ Continuing toward goal (1/20): <judge's reason>` and Hermes takes the next step automatically. For `/goal_prompt_oneshot`, the continuation line preserves control-plane facts with reasoning before the verdict: `judge: <reasoning> → CONTINUE; GOAL.md definition of done: NOT SATISFIED; next action: <exact NEXT_ACTIONS.md slice>`. STOP_FOR_OPERATOR uses the same ordering: `judge: <reasoning> → STOP_FOR_OPERATOR`.
