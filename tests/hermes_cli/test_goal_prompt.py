...
def test_goal_prompt_document_paths_preserves_prompt_and_referenced_docs(tmp_path: Path):
    prompt = tmp_path / "docs" / "runbooks" / "GOAL_PROMPT.md"
    next_actions = tmp_path / "docs" / "status" / "NEXT_ACTIONS.md"
    goal = tmp_path / "GOAL.md"
    readme = tmp_path / "README.md"
    for path in (prompt, next_actions, goal, readme):
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(path.name, encoding="utf-8")

    prompt_text = f"""Routine docs:
- {next_actions}
- {goal}
Operator docs:
- {readme}
"""
    ...
    paths = goal_prompt_document_paths(prompt, prompt_text)
