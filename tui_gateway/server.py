                                ...
                                try:
                                    ...
                                    if (
                                        ...
                                        new_state = new_mgr.set(
                                            goal_text,
                                            max_turns=int(oneshot_cfg.get("oneshot_max_turns", 250) or 250),
                                            goal_mode="goal_prompt_oneshot",
                                            goal_prompt_path=prompt_path,
                                            compaction_refresh_interval=interval,
                                        )
                                        new_state.turns_used = int(getattr(state, "turns_used", 0) or 0)
                                        from hermes_cli.goals import save_goal
                                        save_goal(new_key, new_state)
...
def _(rid, params: dict) -> dict:
    ...
    if name in {"goal_prompt", "goal-prompt", "goal_prompt_oneshot", "goal-prompt-oneshot"}:
        ...
        try:
            ...
            oneshot_max_turns = int(goals_cfg.get("oneshot_max_turns", 250) or 250)
            refresh_interval = int(goals_cfg.get("oneshot_compaction_refresh_interval", 5) or 0)
        except Exception:
            default_max_turns, oneshot_max_turns, refresh_interval = 20, 250, 5
